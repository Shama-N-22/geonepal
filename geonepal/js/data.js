...
const PROVINCES = {
  "Koshi": ["Jhapa","Morang","Sunsari","Ilam","Dhankuta","Bhojpur","Taplejung","Sankhuwasabha","Terhathum","Panchthar","Udayapur","Khotang","Okhaldhunga"],
  "Madhesh": ["Saptari","Siraha","Dhanusha","Mahottari","Sarlahi","Rautahat","Bara","Parsa"],
  "Bagmati": ["Kathmandu","Lalitpur","Bhaktapur","Kavrepalanchok","Sindhupalchok","Nuwakot","Dhading","Rasuwa","Chitwan","Makwanpur","Ramechhap","Dolakha","Sindhuli"],
  "Gandaki": ["Kaski","Lamjung","Tanahun","Syangja","Gorkha","Manang","Mustang","Parbat","Baglung","Nawalpur"],
  "Lumbini": ["Rupandehi","Kapilvastu","Dang","Banke","Bardiya","Palpa","Gulmi","Arghakhanchi","Pyuthan","Rolpa","Nawalparasi (W)"],
  "Karnali": ["Surkhet","Dailekh","Jajarkot","Kalikot","Jumla","Mugu","Humla","Dolpa","Salyan"],
  "Sudurpashchim": ["Kailali","Kanchanpur","Dadeldhura","Doti","Achham","Baitadi","Darchula","Bajura","Bajhang"]
};
...
const TYPE_META = {
  ...
  earthquake: { label:"Earthquakes", tag:"EARTHQUAKE", color:"var(--quake)", icon:"🌍" },
  landslide:  { label:"Landslides", tag:"LANDSLIDE", color:"var(--slide)", icon:"⛰" }
};
...
function provinceOf(district){
  for(const [p, ds] of Object.entries(PROVINCES)) if(ds.includes(district)) return p;
  return "Bagmati";
}
...
function genDemoIncidents(type, year, count){
  ...
  for(let i=0;i<count;i++){
    ...
    out.push({
      id, disasterType:type, year, date,
      country:"Nepal", province, district,
      municipality:`${district} Municipality`,
      severity,
      title:`${type==='flood'?'Flood':'Landslide'} incident — ${district}`,
      description: pick(notes, rnd),
      lat: 26.5 + rnd()*4.3, lng: 80.2 + rnd()*8.6,
      image: `https://picsum.photos/seed/${seedImg}/400/400`,
      gallery:[1,2,3].map(n=>`https://picsum.photos/seed/${seedImg}-g${n}/500/340`),
      demo:true,
    ...
    COMMONS_CACHE[disasterType] = pages
      ...
      .map(p => {
        ...
        return {
          ...
          artist: (meta.Artist && meta.Artist.value) ? meta.Artist.value.replace(/<[^>]+>/g,'') : 'Unknown',
          license: (meta.LicenseShortName && meta.LicenseShortName.value) || 'See Commons page'
        };
      })
      .filter(img => { // dedupe identical files (some Commons categories list the same image twice)
...
async function loadWikiSummary(disasterType){
  ...
  try{
    ...
    if(!res.ok) throw new Error('bad status');
    const j = await res.json();
    WIKI_CACHE[disasterType] = { extract: j.extract, url: j.content_urls && j.content_urls.desktop && j.content_urls.desktop.page, title: j.title };
  }catch(e){
...
/* ---------- honest "no photo" placeholder for real records with no image source ----------
   Built fresh per-incident (embedding that record's own real district, date,
   and verification status) instead of one static image reused everywhere —
   that static reuse was the main reason the grid looked like it was showing
   the same image over and over across hundreds of different real incidents. */
...
const PROVINCE_CENTERS = {
  "Sudurpashchim": [29.3, 80.6], "Karnali": [29.0, 82.3], "Lumbini": [27.9, 82.5],
  "Gandaki": [28.4, 83.9], "Bagmati": [27.9, 85.4], "Madhesh": [26.9, 85.9], "Koshi": [27.3, 87.3]
};
function classifyProvince(lat, lng){
  let best = "Bagmati", bestDist = Infinity;
...
async function loadBipadHazard(disasterType, {maxPages=3, pageLimit=200, sinceDate='2020-01-01'}={}){
  ...
  try{
    while(url && pages < maxPages){
      ...
      (j.results || []).forEach(r => { if(r && r.id != null) seenIds.set(r.id, r); });
      url = j.next || null;
      pages++;
    }
  }catch(e){
    console.warn(`BIPAD fetch failed for hazard ${hazard} (${disasterType})`, e);
    return [];
  }
  ...
  return all
    ...
    .map(r=>{
      ...
      const place = (r.title || '').replace(/^(Earthquake|Flood|Landslide)\s+at\s+/i, '').trim() || r.streetAddress || 'Location not specified';
      const realDistrict = matchRealDistrict(r.title) || matchRealDistrict(place);
      const factParts = [];
      factParts.push(r.verified ? 'Verified by BIPAD.' : 'Not yet verified by BIPAD.');
      if(r.dataSource) factParts.push(`Reported via ${r.dataSource.replace(/_/g,' ')}.`);
      if(r.source) factParts.push(`Source type: ${r.source}.`);
      if(r.wards && r.wards.length) factParts.push(`Ward ID ${r.wards.join(', ')}.`);
      return {
        id:`bipad-${r.id}`, disasterType, year: date.getFullYear(), date,
        country:"Nepal", province,
        district: realDistrict || place,
        municipality: place,
        severity: null,
        verified: !!r.verified,
        ...
        lat, lng,
        image: noPhotoCard(realDistrict || place, date.toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})),
        gallery:[],
        demo:false, source:'BIPAD',
        bipadUrl: `${BIPAD_BASE_URL}/incident/${r.id}/`,
        newsCount:1, videoCount:0, imageCount:0
      };
    })
...
async function loadGDACS(){
  const url = "https://www.gdacs.org/gdacsapi/api/events/geteventlist/SEARCH?fromdate=2020-01-01&todate=2026-12-31&country=157&eventtypes=FL";
  try{
    const res = await fetch(url);
    if(!res.ok) throw new Error('bad status');
    const j = await res.json();
    const feats = (j.features || j) || [];
    const mapped = feats.map((f,i)=>{
      const p = f.properties || f;
      const geom = f.geometry || {};
      const [lng,lat] = geom.coordinates || [84 + Math.random()*4, 27 + Math.random()*2];
      const t = new Date(p.fromdate || p.eventdate || Date.now());
      const alert = (p.alertlevel||'').toLowerCase();
      let severity = "Low";
      if(alert==='red') severity="Critical"; else if(alert==='orange') severity="Severe"; else if(alert==='green') severity="Moderate";
      const nearestDistrict = ALL_DISTRICTS[Math.floor(Math.abs(lat*lng*1000))%ALL_DISTRICTS.length];
      const province = provinceOf(nearestDistrict);
      return {
        id:`gdacs-fl-${p.eventid||i}`, disasterType:'flood', year:t.getFullYear(),
        date:t, country:"Nepal", province, district:nearestDistrict,
        municipality:p.name || nearestDistrict,
        severity, title:p.name || p.htmldescription || `Flood event`,
        description:(p.description || p.htmldescription || 'Flood event recorded by GDACS (Global Disaster Alert and Coordination System).').replace(/<[^>]+>/g,''),
        lat, lng,
        image: noPhotoCard(nearestDistrict, t.toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})),
        gallery:[],
        demo:false, gdacsUrl:p.url && p.url.details ? p.url.details : `https://www.gdacs.org/report.aspx?eventid=${p.eventid}&eventtype=FL`,
        newsCount:1, videoCount:0, imageCount:0
      };
    }).filter(d=>YEARS.includes(d.year));
    if(mapped.length){ DB.flood = mapped; gdacsLoaded = true; }
    else throw new Error('no GDACS results for Nepal in range');
  }catch(e){
    console.warn('GDACS fetch failed or returned nothing, keeping demo flood data', e);
  }
}

/* ---------- real USGS earthquake fetch ---------- */
async function loadUSGS(){
  const url = "https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&starttime=2020-01-01&endtime=2026-12-31&minlatitude=26&maxlatitude=31&minlongitude=80&maxlongitude=89&minmagnitude=2.5&limit=2000";
  try{
    const res = await fetch(url);
    if(!res.ok) throw new Error('bad status');
    const j = await res.json();
    const feats = j.features || [];
    DB.earthquake = feats.map((f,i)=>{
      const mag = f.properties.mag || 0;
      const t = new Date(f.properties.time);
      const [lng,lat,depth] = f.geometry.coordinates;
      let severity = "Low";
      if(mag>=6) severity="Critical"; else if(mag>=5) severity="Severe"; else if(mag>=4) severity="Moderate";
      const nearestDistrict = ALL_DISTRICTS[Math.floor((Math.abs(lat*lng*1000))%ALL_DISTRICTS.length)];
      const province = provinceOf(nearestDistrict);
      return {
        id:`eq-${f.id||i}`, disasterType:'earthquake', year:t.getFullYear(),
        date:t, country:"Nepal", province, district:nearestDistrict,
        municipality:f.properties.place || nearestDistrict,
        severity, title:f.properties.place || `M${mag.toFixed(1)} earthquake`,
        ...
        lat, lng, mag, depth,
        image: noPhotoCard(`M${mag.toFixed(1)} · ${nearestDistrict}`, t.toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})),
        gallery:[],
        demo:false, usgsUrl: f.properties.url,
        newsCount:1, videoCount:0, imageCount:0
      };
    }).filter(d=>YEARS.includes(d.year));
  }catch(e){
    console.warn('USGS fetch failed, falling back to demo quake data', e);
    DB.earthquake = YEARS.flatMap(y=>genDemoIncidents('earthquake', y, 12).map(d=>({...d, demo:true, mag:(2.5+Math.random()*4).toFixed(1)})));
  }
  usgsLoaded = true;
}
...
async function loadCategoryVideos(disasterType){
  if(!ytApiKey){ MEDIA[disasterType].videoLoaded = true; return; }
...
async function loadLiveData(){
  ...
  if(bipadFloods.length){ DB.flood = bipadFloods; }
  else { await loadGDACS(); }

  await loadUSGS();
  const bipadQuakes = await loadBipadHazard('earthquake');
  if(bipadQuakes.length) DB.earthquake = [...DB.earthquake.filter(d=>!d.demo), ...bipadQuakes];
}

function buildDemoData(){
  DB.flood = YEARS.flatMap(y=>genDemoIncidents('flood', y, 70 + Math.floor(Math.random()*40)));
  DB.landslide = YEARS.flatMap(y=>genDemoIncidents('landslide', y, 40 + Math.floor(Math.random()*30)));
}
...
function expandTiles(incidents){
  ...
  incidents.forEach(inc=>{
    ...
    if(inc.demo){
      ...
      for(let i=0;i<ic;i++) tiles.push({...inc, contentType:'image', tileImg:`https://picsum.photos/seed/${inc.id}-img${i}/380/380`});
      const nc = inc.newsCount || 1;
      for(let i=0;i<nc;i++) tiles.push({...inc, contentType:'news', tileImg:`https://picsum.photos/seed/${inc.id}-news${i}/380/380`});
      const vc = inc.videoCount || 0;
      for(let i=0;i<vc;i++) tiles.push({...inc, contentType:'video', tileImg:`https://picsum.photos/seed/${inc.id}-vid${i}/380/380`});
    }
  });
  return tiles;
}
...
function mediaTilesFor(disasterType, year){
  ...
  m.news.forEach(a=>{
    ...
    tiles.push({
      id:a.id, disasterType, contentType:'news', demo:false, isMedia:true,
      title:a.title, district:a.domain, date:a.date || new Date(),
      tileImg: a.image || NO_PHOTO_SVG, mediaData:a
    });
  });
  m.video.forEach(v=>{
    if(year!=='all' && (!v.date || v.date.getFullYear()!==year)) return;
    tiles.push({
      id:v.id, disasterType, contentType:'video', demo:false, isMedia:true,
      title:v.title, district:v.channel, date:v.date || new Date(),
      tileImg: v.thumb, mediaData:v
    });
  });
  (COMMONS_CACHE[disasterType]||[]).forEach((img,i)=>{
    tiles.push({
      id:`commons-${disasterType}-${i}`, disasterType, contentType:'image', demo:false, isMedia:true,
      title:'Reference photograph — Wikimedia Commons', district:img.artist, date: null,
      tileImg: img.url, mediaData:img
    });
  });
  return tiles;
}