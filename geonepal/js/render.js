...
function getFilteredTiles(){
  ...
  if(state.year !== 'all') incidents = incidents.filter(d=>d.year===state.year);
  if(state.province !== 'all') incidents = incidents.filter(d=>d.province===state.province);
  if(state.district !== 'all') incidents = incidents.filter(d=>d.district===state.district);
  if(state.severity !== 'all') incidents = incidents.filter(d=>d.severity===state.severity);
  let tiles = expandTiles(incidents);

  // Real category-level media (news/video/image) — only applies to filters
  // that could show them, and province/district/severity don't apply since
  // this content isn't tied to one incident's location record.
  if(state.province==='all' && state.district==='all' && state.severity==='all'){
    tiles = tiles.concat(mediaTilesFor(state.disaster, state.year));
  }

  if(state.contentType !== 'all') tiles = tiles.filter(t=>t.contentType===state.contentType);
...
function renderYearRail(){
  ...
  allChip.onclick = ()=>{ state.year='all'; state.visibleCount=150; renderYearRail(); renderStats(); renderGrid(); };
  ...
  YEARS.slice().reverse().forEach(y=>{
    ...
    chip.onclick = ()=>{ state.year=y; state.visibleCount=150; renderYearRail(); renderStats(); renderGrid(); };
...
function renderStats(){
  ...
  const images = media.filter(m=>m.contentType==='image').length + incidents.reduce((a,d)=>a+(d.demo?(d.imageCount||0):0),0);
  const videos = media.filter(m=>m.contentType==='video').length + incidents.reduce((a,d)=>a+(d.demo?(d.videoCount||0):0),0);
  const news = media.filter(m=>m.contentType==='news').length + incidents.reduce((a,d)=>a+(d.demo?(d.newsCount||0):0),0);
  const cells = [
    ['Incidents', incidents.length], ['Districts', districts], ['Images', images],
    ['Videos', videos], ['News Reports', news], ['Provinces', provinces]
  ];
  bar.innerHTML = cells.map(([label,num])=>`
    <div class="stat-cell"><div class="stat-num" data-target="${num}">0</div><div class="stat-label">${label}</div></div>
  `).join('');
  bar.querySelectorAll('.stat-num').forEach(el=>animateNum(el, +el.dataset.target));
}

/* ---------- toolbar filters ---------- */
function renderProvinceOptions(){
  const sel = document.getElementById('provinceSelect');
  sel.innerHTML = '<option value="all">All Provinces</option>' + Object.keys(PROVINCES).map(p=>`<option value="${p}">${p}</option>`).join('');
}
function renderDistrictOptions(){
  const sel = document.getElementById('districtSelect');
  const list = state.province==='all' ? ALL_DISTRICTS : PROVINCES[state.province];
...
function renderGrid(){
  ...
  document.getElementById('resultCount').textContent = `${tiles.length.toLocaleString()} RESULTS`;
  if(!tiles.length){
  ...
  grid.innerHTML = shown.map(t=>{
    ...
    const typeIcon = {incident:'📍',image:'📷',news:'📰',video:'▶'}[t.contentType];
  ...
  document.getElementById('loadMoreBtn').style.display = tiles.length>state.visibleCount ? 'inline-block' : 'none';
}
...
function renderMap(){
  ...
  if(!mainLeafletMap){
    mainLeafletMap = L.map('leafletMap', { preferCanvas:true }).setView([28.3949, 84.1240], 7);
  ...
  const colorVar = state.disaster==='flood' ? '--flood' : state.disaster==='earthquake' ? '--quake' : '--slide';
  const color = getComputedStyle(document.documentElement).getPropertyValue(colorVar).trim();

  // Perf cap — plain (non-clustered) markers can bog down the browser past a
  // few hundred points, so we cap the render rather than freeze the map.
  const CAP = 500;
  const shown = incidents.slice(0, CAP);
  shown.forEach(inc=>{
    if(typeof inc.lat !== 'number' || typeof inc.lng !== 'number') return;
    const marker = L.circleMarker([inc.lat, inc.lng], {
      radius: 6, color: color, weight: 1.5, fillColor: color, fillOpacity: 0.55
    });
    const verifiedLine = inc.verified!==undefined ? (inc.verified ? ' · ✓ Verified' : ' · Unverified') : (inc.mag ? ` · M${(+inc.mag).toFixed(1)}` : '');
    marker.bindPopup(`<strong>${inc.title}</strong><br>${inc.district}, ${inc.province}<br>${inc.date.toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})}${verifiedLine}<br><a href="#" style="color:${color}">View full details →</a>`);
    marker.on('popupopen', (e)=>{
      const link = e.popup._contentNode.querySelector('a');
      if(link) link.onclick = (ev)=>{ ev.preventDefault(); openModal(inc); };
    });
...
function renderTimeline(){
  ...
  const meta = TYPE_META[state.disaster];
  rail.innerHTML = YEARS.slice().reverse().map(y=>{
    const incidents = (DB[state.disaster]||[]).filter(d=>d.year===y);
    let top = [...incidents].sort((a,b)=> (b.mag||0)-(a.mag||0) || 0).slice(0,3);
  ...
  rail.querySelectorAll('.tev[data-id]').forEach(el=>{
    el.onclick = ()=>{
      const inc = (DB[state.disaster]||[]).find(d=>d.id===el.dataset.id);
      if(inc) openModal(inc);
    };
  });
}
...
async function openModal(inc){
  ...
  const stages = inc.disasterType==='earthquake'
    ...
    <div class="modal-body">
      ...
      <div class="modal-block">
        ...
        <div class="video-grid" id="videoGrid_${inc.id}">
          ${ytApiKey ? `<div style="grid-column:1/-1;font-family:var(--font-mono);font-size:11px;color:var(--text-2)">Loading real videos via YouTube API…</div>` : `
  ...
  if(!inc.demo){
    ...
    if(gdeltBlock && gdeltContent){
      const catNews = (MEDIA[inc.disasterType] && MEDIA[inc.disasterType].news) || [];
      if(!catNews.length){
        ...
        gdeltBlock.style.display = 'block';
        gdeltContent.innerHTML = catNews.slice(0,3).map(a=>`
          <div class="news-item">
            <div><div class="nh">${a.title}</div><div class="ns">${a.domain}${a.date ? ' · '+a.date.toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'}) : ''}</div></div>
            <a class="news-btn" href="${a.url}" target="_blank">READ ARTICLE →</a>
          </div>
        `).join('') + `<div class="demo-note" style="margin-top:10px;color:var(--text-2);border-color:var(--panel-border);background:none;">◆ Showing general ${meta.label.toLowerCase()} coverage, not necessarily this exact incident — see the NEWS filter tab for the full list.</div>`;
      }
    }
  }
  ...
  if(typeof inc.lat === 'number' && typeof inc.lng === 'number'){
    setTimeout(()=>{
      ...
      const miniMap = L.map(container, { zoomControl:false, attributionControl:false, scrollWheelZoom:false }).setView([inc.lat, inc.lng], 11);
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom:18 }).addTo(miniMap);
      L.circleMarker([inc.lat, inc.lng], { radius:8, color: meta.color, weight:2, fillColor: meta.color, fillOpacity:0.6 }).addTo(miniMap);
...
function openMediaModal(item){
  ...
  const kindLabel = {news:'NEWS ARTICLE', video:'VIDEO', image:'REFERENCE PHOTOGRAPH'}[item.contentType];
  const sourceLabel = {news:'via GDELT news index', video:'via YouTube', image:'via Wikimedia Commons'}[item.contentType];
...
async function fetchYouTube(inc, container){
  try{
    const q = encodeURIComponent(`${inc.district} Nepal ${inc.disasterType} ${inc.year}`);
    const url = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=4&q=${q}&key=${ytApiKey}`;
    const res = await fetch(url);
    const j = await res.json();
    if(j.error){ container.innerHTML = `<div style="grid-column:1/-1;font-family:var(--font-mono);font-size:11px;color:#ff5a3c">YouTube API error: ${j.error.message}</div>`; return; }
    const items = j.items || [];
    if(!items.length){ container.innerHTML = `<div style="grid-column:1/-1;font-family:var(--font-mono);font-size:11px;color:var(--text-2)">No real videos found for this query.</div>`; return; }
